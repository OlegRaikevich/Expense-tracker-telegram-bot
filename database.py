...
def get_expenses_for_period(user_id, period):
    ...
    cursor.execute("SELECT SUM(amount) FROM expenses WHERE user_id = ? AND timestamp >= ?", (user_id, start_date))
...
def get_average_expense(user_id):
    conn = sqlite3.connect('expenses.db')
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(DISTINCT DATE(timestamp)), SUM(amount) FROM expenses WHERE user_id = ?", (user_id,))
